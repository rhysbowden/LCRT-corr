...
# rev5: now stores AIC and BIC values for each iteration
# rev6: - stores variance components
#       - parses warnings
#       - excludes any fit that has a warning associated with it if disqualify_warnings=TRUE
...
resuming = FALSE # if you set this to true, also set summary_fname and details_fname below.
maxit1=50
disqualify_warnings = TRUE

# design and model parameters. You can specify any of these (except sigma_eps2) as a list.
T_list = c(3,4,5,6,10) # number of periods
c_list = c(20,50,100) # Number of clusters in total
N_list = c(100,50,10) # number of individual observations per cluster-period
icc_list = c(0.01,0.05,0.1,0.2) # intra-cluster correlation
cac_list = c(2-0.5,2-0.8, 2-0.95, 1, 0.95, 0.8, 0.5) # cluster auto-correlation, r in Jess' misspecification paper. Values greater than 1 indicate BE model instead of DTD model and will be transformed by 1-value later.
...
if(resuming){ # set these two file names if you are resuming from a previous partially completed run
  summary_fname = '2021-05-12_21-30-33_1.txt'
  if(store_each_replicate){
    details_fname = '2021-05-12_21-30-33_1_details.txt'
  }
  error_fname = '2021-05-12_21-30-33_1_err.txt'
...
if(resuming==FALSE){
  ...
  if(store_each_replicate){
    setting_full_df = data.frame(
      ...
      AIC_DTD = numeric(),
      BIC_DTD = numeric(),
      DTDclus_time_corr = numeric(),
      DTDclus_time_var = numeric(),
      DTDunits = numeric(),
      DTDR = numeric(),
      HHclus = numeric(),
      HHunits = numeric(),
      HHR = numeric(),
      BEclus_time_corr = numeric(),
      BEclus_time_var = numeric(),
      BEunits = numeric(),
      BER = numeric()
    )
...
for (param_ind in starting_setting:num_param_tuples) {
  ...
  AIC_HH = array(NA,dim=c(1,num_repeats))
  BIC_HH = array(NA,dim=c(1,num_repeats))
  AIC_BE = array(NA,dim=c(1,num_repeats))
  BIC_BE = array(NA,dim=c(1,num_repeats))
  AIC_DTD = array(NA,dim=c(1,num_repeats))
  BIC_DTD = array(NA,dim=c(1,num_repeats))
  var_comps = array(NA,dim=c(11,num_repeats))
  ...
  for(i in 1:num_repeats){
    ...
    if(Generation_model =='BE' | Generation_model =='HH'){
      ...
      if(Generation_model =='BE'){
        YY1 =  eps +aV +betaV +nuV  # cluster-period cell mean level
        YY1i = epsi+aVi+betaVi+nuVi # individual level version
      }
      if(Generation_model =='HH'){
        YY1 =  eps +aV +betaV  # cluster-period cell mean level
        YY1i = epsi+aVi+betaVi # individual level version
      }
    }
    ...
    if(individual_level){
      Y1 = YY1i
      Ydf1 = data.frame(YY1i,timef,clusterf,clustimef)
    }else{
      Y1 = YY1
      Ydf1 = data.frame(YY1,timef,clusterf,clustimef)
    }
    ...
    fitY1_DTD = NULL 
    output1 = tryCatch({
      fitY1_DTD = asreml(fixed= Y1 ~ timef, random= ~id(clusterf):ar1v(timef),
                         residual=~idv(units),data=Ydf1,maxit = maxit1,trace=F)
      list(fitY1_DTD,0) 
    },
    warning=function(war){
      ...
      war_vec = str_detect(war,c("Some components changed","not converged","Iteration terminated"))
      war_num = 1+sum(c(0.5,0.25,0.125)*war_vec)
      return(list(fitY1_DTD,war_num))
    },
    error=function(err){
    ...
    if(!is.null(fitY1_DTD)){
      var_comps[1:4,i] = summary(fitY1_DTD)$varcomp[1:4,1]
    }
    
    fitY1_BE = NULL 
    output2 = tryCatch({
      fitY1_BE = asreml(fixed= Y1 ~ timef, random= ~id(clusterf):corv(timef),
                        residual=~idv(units),data=Ydf1,maxit = maxit1, trace=F)
      list(fitY1_BE,0)
    },
    warning=function(war){
    ...
    if(!is.null(fitY1_BE)){
      var_comps[5:7,i] = summary(fitY1_BE)$varcomp[1:3,1]
    }
    
    fitY1_HH =NULL
    output3 = tryCatch({
      fitY1_HH = asreml(fixed= Y1 ~ timef, random= ~id(clusterf),
                        residual=~idv(units),data=Ydf1,maxit = maxit1, trace=F)
      list(fitY1_HH,0)
    },
    warning=function(war){
    ...
    if(!is.null(fitY1_HH)){
      var_comps[8:11,i] = summary(fitY1_HH)$varcomp[1:4,1]
    }
    ...
    if(disqualify_warnings){
      AICs[c(status_HH[1,i],status_BE[1,i],status_DTD[1,i])>=1]=NA
    }
    ...
    if(disqualify_warnings){
      BICs[c(status_HH[1,i],status_BE[1,i],status_DTD[1,i])>=1]=NA
    }
  ...
  warning_HH_m[param_ind] <- sum(floor(status_HH[1,])==1)
  warning_BE_m[param_ind] <- sum(floor(status_BE[1,])==1)
  warning_DTD_m[param_ind] <- sum(floor(status_DTD[1,])==1)
  ...
  if(store_each_replicate){
    setting_full_df = data.frame(
      ...
      AIC_DTD= AIC_DTD[1,],
      BIC_DTD= BIC_DTD[1,],
      DTDclus_time_corr = var_comps[1,],
      DTDclus_time_var = numeric[2,],
      DTDunits = numeric[3,],
      DTDR = numeric[4,],
      HHclus = numeric[5,],
      HHunits = numeric[6,],
      HHR = numeric[7,],
      BEclus_time_corr = numeric[8,],
      BEclus_time_var = numeric[9,],
      BEunits = numeric[10,],
      BER = numeric[11,]
    )
    write.table( setting_full_df,  
